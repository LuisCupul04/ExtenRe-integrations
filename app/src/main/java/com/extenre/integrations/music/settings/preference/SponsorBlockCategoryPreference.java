...
import com.extenre.integrations.music.sponsorblock.objects.CategoryBehaviour;
...
public class SponsorBlockCategoryPreference {
    private static final String[] CategoryBehaviourEntries = {str("extenre_sb_skip_automatically"), str("extenre_sb_skip_ignore")};
    ...
    public static void showDialog(Activity baseActivity, String categoryString) {
        try {
            ...
            colorTextLabel.setText(str("extenre_sb_color_dot_label"));
            ...
            builder.setPositiveButton(android.R.string.ok, (dialog, which) -> {
                ...
                try {
                    if (!colorString.equals(category.colorString())) {
                        ...
                        Utils.showToastShort(str("extenre_sb_color_changed"));
                    }
                } catch (IllegalArgumentException ex) {
                    Utils.showToastShort(str("extenre_sb_color_invalid"));
                }
            });
            builder.setNeutralButton(str("extenre_sb_reset_color"), (dialog, which) -> {
                try {
                    category.resetColor();
                    Utils.showToastShort(str("extenre_sb_color_reset"));
                } catch (Exception ex) {

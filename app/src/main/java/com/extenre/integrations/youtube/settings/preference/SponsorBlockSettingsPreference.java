...
import com.extenre.integrations.shared.settings.preference.ResettableEditTextPreference;
...
import com.extenre.integrations.youtube.sponsorblock.SponsorBlockUtils;
import com.extenre.integrations.youtube.sponsorblock.objects.SegmentCategory;
import com.extenre.integrations.youtube.sponsorblock.objects.UserStats;
import com.extenre.integrations.youtube.sponsorblock.requests.SBRequester;
import com.extenre.integrations.youtube.sponsorblock.ui.SponsorBlockViewController;

@SuppressWarnings({"unused", "deprecation"})
public class SponsorBlockSettingsPreference extends ExtenRePreferenceFragment {

    private static PreferenceCategory statsCategory;

    private static final int preferencesCategoryLayout = getLayoutIdentifier("extenre_settings_preferences_category");
    ...
    public static void init(Activity mActivity) {
        ...
        showSkipToast.setOnPreferenceClickListener(preference -> {
            Utils.showToastShort(str("extenre_sb_skipped_sponsor"));
            return false;
        });
        ...
        addNewSegment.setOnPreferenceChangeListener((preference, newValue) -> {
            if ((Boolean) newValue && !Settings.SB_SEEN_GUIDELINES.get()) {
                ...
                new AlertDialog.Builder(context)
                        .setTitle(str("extenre_sb_guidelines_popup_title"))
                        .setMessage(str("extenre_sb_guidelines_popup_content"))
                        .setNegativeButton(str("extenre_sb_guidelines_popup_already_read"), null)
                        .setPositiveButton(str("extenre_sb_guidelines_popup_open"), (dialogInterface, i) -> openGuidelines(context))
        ...
        newSegmentStep.setOnPreferenceChangeListener((preference, newValue) -> {
            ...
            Utils.showToastLong(str("extenre_sb_general_adjusting_invalid"));
            updateUI();
            return false;
        });
        final Preference guidelinePreferences = Objects.requireNonNull(mPreferenceManager.findPreference("extenre_sb_guidelines_preference"));
        ...
        minSegmentDuration.setOnPreferenceChangeListener((preference, newValue) -> {
            ...
            Utils.showToastLong(str("extenre_sb_general_min_duration_invalid"));
            updateUI();
            return false;
        });
        ...
        privateUserId.setOnPreferenceChangeListener((preference, newValue) -> {
            ...
            if (!SponsorBlockSettings.isValidSBUserId(newUUID)) {
                Utils.showToastLong(str("extenre_sb_general_uuid_invalid"));
                return false;
            }
            ...
            apiUrl.setOnPreferenceClickListener(preference -> {
                ...
                DialogInterface.OnClickListener urlChangeListener = (dialog, buttonPressed) -> {
                    if (buttonPressed == DialogInterface.BUTTON_NEUTRAL) {
                        ...
                        Utils.showToastLong(str("extenre_sb_api_url_reset"));
                        ...
                        if (!SponsorBlockSettings.isValidSBServerAddress(serverAddress)) {
                            Utils.showToastLong(str("extenre_sb_api_url_invalid"));
                        } else if (!serverAddress.equals(Settings.SB_API_URL.get())) {
                            Settings.SB_API_URL.save(serverAddress);
                            Utils.showToastLong(str("extenre_sb_api_url_changed"));
                        }
                    }
                };
                Utils.getEditTextDialogBuilder(context)
                        ...
                        .setNeutralButton(str("extenre_sb_reset"), urlChangeListener)
        ...
        statsCategory.setTitle(str("extenre_sb_stats"));
        ...
        aboutCategory.setTitle(str("extenre_sb_about"));
        ...
        aboutPreference.setTitle(str("extenre_sb_about_api"));
        aboutPreference.setSummary(str("extenre_sb_about_api_sum"));
    ...
    public static void fetchAndDisplayStats() {
        try {
            ...
            if (Settings.SB_ENABLED.get()) {
                loadingPlaceholderPreference.setTitle(str("extenre_sb_stats_loading"));
                ...
                loadingPlaceholderPreference.setTitle(str("extenre_sb_stats_sb_disabled"));
            }
        } catch (Exception ex) {
    ...
    private static void addUserStats(@NonNull Preference loadingPlaceholder, @Nullable UserStats stats) {
        ...
        try {
            if (stats == null) {
                loadingPlaceholder.setTitle(str("extenre_sb_stats_connection_failure"));
                return;
            }
            statsCategory.removeAll();
            Context context = statsCategory.getContext();
            ...
            if (stats.totalSegmentCountIncludingIgnored > 0) {
                ...
                preference.setTitle(fromHtml(str("extenre_sb_stats_username", userName)));
                preference.setSummary(str("extenre_sb_stats_username_change"));
                    ...
                    Utils.runOnBackgroundThread(() -> {
                        ...
                        Utils.runOnMainThread(() -> {
                            if (errorMessage == null) {
                                preference.setTitle(fromHtml(str("extenre_sb_stats_username", newUserName)));
                                preference.setText(newUserName);
                                Utils.showToastLong(str("extenre_sb_stats_username_changed"));
                            } else {
...

            {
                ...
                preference.setTitle(fromHtml(str("extenre_sb_stats_submissions", formatted)));
                preference.setSummary(str("extenre_sb_stats_submissions_sum"));
                if (stats.totalSegmentCountIncludingIgnored == 0) {
...

            {
                ...
                Preference preference = new Preference(context);
                preference.setTitle(fromHtml(str("extenre_sb_stats_reputation", stats.reputation)));
                preference.setSelectable(false);
...

            {
                ...
                if (stats.totalSegmentCountIncludingIgnored == 0) {
                    stats_saved = str("extenre_sb_stats_saved_zero");
                    stats_saved_sum = str("extenre_sb_stats_saved_sum_zero");
                } else {
                    stats_saved = str("extenre_sb_stats_saved",
                            SponsorBlockUtils.getNumberOfSkipsString(stats.viewCount));
                    stats_saved_sum = str("extenre_sb_stats_saved_sum", SponsorBlockUtils.getTimeSavedString((long) (60 * stats.minutesSaved)));
                }
    ...
    private static void addLocalUserStats() {
        ...
        Runnable updateStatsSelfSaved = () -> {
            ...
            preference.setTitle(fromHtml(str("extenre_sb_stats_self_saved", formatted)));
            String formattedSaved = SponsorBlockUtils.getTimeSavedString(Settings.SB_LOCAL_TIME_SAVED_MILLISECONDS.get() / 1000);
            preference.setSummary(fromHtml(str("extenre_sb_stats_self_saved_sum", formattedSaved)));
        };
        updateStatsSelfSaved.run();
        preference.setOnPreferenceClickListener(preference1 -> {
            new AlertDialog.Builder(preference1.getContext())
                    .setTitle(str("extenre_sb_stats_self_saved_reset_title"))

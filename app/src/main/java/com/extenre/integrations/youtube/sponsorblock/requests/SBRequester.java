package com.extenre.integrations.youtube.sponsorblock.requests;
...
import com.extenre.integrations.shared.sponsorblock.requests.SBRoutes;
...
public class SBRequester {
    ...
    public static SponsorSegment[] getSegments(@NonNull String videoId) {
        ...
        try {
            ...
            if (responseCode == HTTP_STATUS_CODE_SUCCESS) {
                ...
                handleConnectionError(str("extenre_sb_sponsorblock_connection_failure_status", responseCode), null);
                connection.disconnect(); // something went wrong, might as well disconnect
            }
        } catch (SocketTimeoutException ex) {
            handleConnectionError(str("extenre_sb_sponsorblock_connection_failure_timeout"), ex);
        } catch (IOException ex) {
            handleConnectionError(str("extenre_sb_sponsorblock_connection_failure_generic"), ex);
        } catch (Exception ex) {
            // Should never happen
    ...
    public static void submitSegments(@NonNull String videoId, @NonNull String category,
        ...
        try {
            ...
            final String messageToToast = switch (responseCode) {
                case HTTP_STATUS_CODE_SUCCESS -> str("extenre_sb_submit_succeeded");
                case 409 -> str("extenre_sb_submit_failed_duplicate");
                case 403 ->
                        str("extenre_sb_submit_failed_forbidden", Requester.parseErrorStringAndDisconnect(connection));
                case 429 -> str("extenre_sb_submit_failed_rate_limit");
                case 400 ->
                        str("extenre_sb_submit_failed_invalid", Requester.parseErrorStringAndDisconnect(connection));
                default ->
                        str("extenre_sb_submit_failed_unknown_error", responseCode, connection.getResponseMessage());
            };
            Utils.showToastLong(messageToToast);
        } catch (SocketTimeoutException ex) {
            // Always show, even if show connection toasts is turned off
            Utils.showToastLong(str("extenre_sb_submit_failed_timeout"));
        } catch (IOException ex) {
            Utils.showToastLong(str("extenre_sb_submit_failed_unknown_error", 0, ex.getMessage()));
        } catch (Exception ex) {
        ...
        Utils.runOnBackgroundThread(() -> {
            try {
                ...
                switch (responseCode) {
                    ...
                    case 403:
                        Utils.showToastLong(
                                str("extenre_sb_vote_failed_forbidden", Requester.parseErrorStringAndDisconnect(connection)));
                        break;
                    default:
                        Utils.showToastLong(
                                str("extenre_sb_vote_failed_unknown_error", responseCode, connection.getResponseMessage()));
                        break;
                }
            } catch (SocketTimeoutException ex) {
                Utils.showToastShort(str("extenre_sb_vote_failed_timeout"));
            } catch (IOException ex) {
                Utils.showToastShort(str("extenre_sb_vote_failed_unknown_error", 0, ex.getMessage()));
            } catch (Exception ex) {
    ...
    public static String setUsername(@NonNull String username) {
        ...
        try {
            ...
            return str("extenre_sb_stats_username_change_unknown_error", responseCode, responseMessage);
        } catch (Exception ex) { // should never happen
            Logger.printInfo(() -> "failed to set username", ex); // do not toast
            return str("extenre_sb_stats_username_change_unknown_error", 0, ex.getMessage());
        }
    }

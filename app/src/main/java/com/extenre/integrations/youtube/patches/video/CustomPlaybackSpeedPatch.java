...
import static com.extenre.integrations.shared.utils.ResourceUtils.getString;
...
import com.extenre.integrations.youtube.patches.components.PlaybackSpeedMenuFilter;
...
public class CustomPlaybackSpeedPatch {
    ...
    static {
        defaultSpeedEntries = new String[]{getString("quality_auto"), "0.25x", "0.5x", "0.75x", getString("extenre_playback_speed_normal"), "1.25x", "1.5x", "1.75x", "2.0x"};
        ...
        try {
            ...
            for (String speedString : speedStrings) {
                ...
                if (speedFloat > MAXIMUM_PLAYBACK_SPEED) {
                    resetCustomSpeeds(str("extenre_custom_playback_speeds_invalid", MAXIMUM_PLAYBACK_SPEED));
                    loadCustomSpeeds();
                    return;
                }
            ...
            for (float speed : playbackSpeeds) {
                ...
                customSpeedEntries[i] = speed != 1.0f
                        ? speedString + "x"
                        : getString("extenre_playback_speed_normal");
                customSpeedEntryValues[i] = speedString;
                i++;
            }
        } catch (Exception ex) {
            Logger.printInfo(() -> "parse error", ex);
            resetCustomSpeeds(str("extenre_custom_playback_speeds_parse_exception"));
            loadCustomSpeeds();
        }
    }

...
package com.extenre.integrations.youtube.utils;
...
import static com.extenre.integrations.youtube.patches.video.PlaybackSpeedPatch.userSelectedPlaybackSpeed;
...
import com.extenre.integrations.shared.utils.IntentUtils;
...
import com.extenre.integrations.youtube.settings.preference.ExternalDownloaderPlaylistPreference;
import com.extenre.integrations.youtube.settings.preference.ExternalDownloaderVideoPreference;
...
public class VideoUtils extends IntentUtils {
    ...
    public static void copyUrl(boolean withTimestamp) {
        setClipboard(getVideoUrl(withTimestamp), withTimestamp
                ? str("extenre_share_copy_url_timestamp_success")
                : str("extenre_share_copy_url_success")
        );
    }

    public static void copyTimeStamp() {
        final String timeStamp = getTimeStamp(VideoInformation.getVideoTime());
        setClipboard(timeStamp, str("extenre_share_copy_timestamp_success", timeStamp));
    }
    ...
    public static void openVideo(@NonNull String videoScheme, @NonNull String prefixId) {
        if (!TextUtils.isEmpty(prefixId)) {
            ...
            if (!channelId.startsWith("UC")) {
                showToastShort(str("extenre_overlay_button_play_all_not_available_toast"));
                return;
            }

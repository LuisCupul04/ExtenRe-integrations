...
package com.extenre.integrations.youtube.sponsorblock;
...
import com.extenre.integrations.youtube.sponsorblock.objects.CategoryBehaviour;
...
import com.extenre.integrations.youtube.sponsorblock.objects.SponsorSegment.SegmentVote;
...
public class SponsorBlockUtils {
        ...
        public void onClick(DialogInterface dialog, int which) {
            try {
                ...
                if (category.behaviour == CategoryBehaviour.IGNORE) {
                    Utils.showToastLong(str("extenre_sb_new_segment_disabled_category"));
                    enableButton = false;
                } else {
        ...
        public void onClick(DialogInterface dialog, int which) {
            try {
                ...
                new AlertDialog.Builder(context)
                        .setTitle(str("extenre_sb_new_segment_choose_category"))
    ...
    private static final DialogInterface.OnClickListener editByHandDialogListener = (dialog, which) -> {
        try {
            ...
            new AlertDialog.Builder(context)
                    .setTitle(str(isStart ? "extenre_sb_new_segment_time_start" : "extenre_sb_new_segment_time_end"))
                    .setView(textView)
                    .setNegativeButton(android.R.string.cancel, null)
                    .setNeutralButton(str("extenre_sb_new_segment_now"), editByHandSaveDialogListener)
    ...
    public static void onMarkLocationClicked() {
        try {
            ...
            new AlertDialog.Builder(SponsorBlockViewController.getOverLaysViewGroupContext())
                    .setTitle(str("extenre_sb_new_segment_title"))
                    .setMessage(str("extenre_sb_new_segment_mark_current_time_as_question",
                            formatSegmentTime(newSponsorSegmentDialogShownMillis)))
                    .setNeutralButton(android.R.string.cancel, null)
                    .setNegativeButton(str("extenre_sb_new_segment_mark_start"), newSponsorSegmentDialogListener)
                    .setPositiveButton(str("extenre_sb_new_segment_mark_end"), newSponsorSegmentDialogListener)
                    .show();
        } catch (Exception ex) {
    ...
    public static void onPublishClicked() {
        try {
            ...
            if (newSponsorSegmentStartMillis < 0 || newSponsorSegmentEndMillis < 0) {
                Utils.showToastShort(str("extenre_sb_new_segment_mark_locations_first"));
            } else if (newSponsorSegmentStartMillis >= newSponsorSegmentEndMillis) {
                Utils.showToastShort(str("extenre_sb_new_segment_start_is_before_end"));
            } else if (!newSponsorSegmentPreviewed && newSponsorSegmentStartMillis != 0) {
                Utils.showToastLong(str("extenre_sb_new_segment_preview_segment_first"));
            } else {
                final long segmentLength = (newSponsorSegmentEndMillis - newSponsorSegmentStartMillis) / 1000;
                new AlertDialog.Builder(SponsorBlockViewController.getOverLaysViewGroupContext())
                        .setTitle(str("extenre_sb_new_segment_confirm_title"))
                        .setMessage(str("extenre_sb_new_segment_confirm_contents",
    ...
    public static void onVotingClicked(@NonNull Context context) {
        try {
            ...
            if (segments == null || segments.length == 0) {
                ...
                Utils.showToastShort(str("extenre_sb_vote_no_segments"));
                return;
            }
    ...
    public static void onEditByHandClicked() {
        try {
            Utils.verifyOnMainThread();
            new AlertDialog.Builder(SponsorBlockViewController.getOverLaysViewGroupContext())
                    .setTitle(str("extenre_sb_new_segment_edit_by_hand_title"))
                    .setMessage(str("extenre_sb_new_segment_edit_by_hand_content"))
                    .setNeutralButton(android.R.string.cancel, null)
                    .setNegativeButton(str("extenre_sb_new_segment_mark_start"), editByHandDialogListener)
                    .setPositiveButton(str("extenre_sb_new_segment_mark_end"), editByHandDialogListener)
                    .show();
        } catch (Exception ex) {
    ...
    public static String getTimeSavedString(long totalSecondsSaved) {
        ...
        if (hours > 0) {
            ...
            return str("extenre_sb_stats_saved_hour_format", hoursFormatted, minutesFormatted);
        }
        final long seconds = duration.getSeconds() % 60;
        String secondsFormatted = statsNumberFormatter.format(seconds);
        if (minutes > 0) {
            return str("extenre_sb_stats_saved_minute_format", minutesFormatted, secondsFormatted);
        }
        return str("extenre_sb_stats_saved_second_format", secondsFormatted);
    }
        ...
        public void onClick(DialogInterface dialog, int which) {
            try {
                ...
                if (which == DialogInterface.BUTTON_NEUTRAL) {
                    ...
                    if (time < 0) {
                        Utils.showToastLong(str("extenre_sb_new_segment_edit_by_hand_parse_error"));
                        return;
                    }
                }

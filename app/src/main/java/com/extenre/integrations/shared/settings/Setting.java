...
public abstract class Setting<T> {
     ...
     */
    public static final SharedPrefCategory preferences = new SharedPrefCategory("extenre");
     ...
     */
    private static final String OPTIONAL_extenre_SETTINGS_PREFIX = "extenre_";

    /**
     * The path, minus any 'extenre' prefix to keep json concise.
     */
    private String getImportExportKey() {
        if (key.startsWith(OPTIONAL_extenre_SETTINGS_PREFIX)) {
            return key.substring(OPTIONAL_extenre_SETTINGS_PREFIX.length());
        }
        return key;
    }
    ...
    public static String exportToJson(@Nullable Context alertDialogContext) {
        try {
            ...
            if (alertDialogContext != null) {
                com.extenre.integrations.youtube.sponsorblock.SponsorBlockSettings.showExportWarningIfNeeded(alertDialogContext);
            }
    ...
    public static boolean importFromJSON(@NonNull String settingsJsonString, boolean isYouTube) {
        try {
            ...
            if (isYouTube) {
                com.extenre.integrations.youtube.sponsorblock.SponsorBlockSettings.updateFromImportedSettings();
            } else {
                com.extenre.integrations.music.sponsorblock.SponsorBlockSettings.updateFromImportedSettings();
            }

            Utils.showToastLong(numberOfSettingsImported == 0
                    ? str("extenre_extended_settings_import_reset")
                    : str("extenre_extended_settings_import_success", numberOfSettingsImported));

            return rebootSettingChanged;
        } catch (JSONException | IllegalArgumentException ex) {
            Utils.showToastLong(str("extenre_extended_settings_import_failed", ex.getMessage()));
            Logger.printInfo(() -> "", ex);
        } catch (Exception ex) {

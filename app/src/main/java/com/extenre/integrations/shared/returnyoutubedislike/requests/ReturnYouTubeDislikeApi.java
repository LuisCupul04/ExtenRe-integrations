...
import static com.extenre.integrations.shared.returnyoutubedislike.requests.ReturnYouTubeDislikeRoutes.getRYDConnectionFromRoute;
...
import com.extenre.integrations.shared.returnyoutubedislike.ReturnYouTubeDislike;
...
public class ReturnYouTubeDislikeApi {
    ...
    private static void updateRateLimitAndStats(boolean connectionError, boolean rateLimitHit) {
        ...
        if (connectionError) {
            ...
            if (!lastApiCallFailed && toastOnConnectionError) {
                Utils.showToastLong(str("extenre_ryd_failure_client_rate_limit_requested"));
            }
            lastApiCallFailed = true;
        } else {
            lastApiCallFailed = false;
    ...
    public static RYDVoteData fetchVotes(String videoId) {
        ...
        try {
            ...
            if (responseCode == HTTP_STATUS_CODE_SUCCESS) {
                ...
                handleConnectionError(str("extenre_ryd_failure_connection_status_code", responseCode), null);
            }
            connection.disconnect(); // something went wrong, might as well disconnect
        } catch (
                SocketTimeoutException ex) { // connection timed out, response timeout, or some other network error
            handleConnectionError((str("extenre_ryd_failure_connection_timeout")), ex);
        } catch (IOException ex) {
            handleConnectionError((str("extenre_ryd_failure_generic", ex.getMessage())), ex);
        } catch (Exception ex) {
            // should never happen
    ...
    public static String registerAsNewUser() {
        ...
        try {
            ...
            handleConnectionError(str("extenre_ryd_failure_generic", "registration failed"), ex);
        } catch (Exception ex) {
    ...
    private static String confirmRegistration(String userId, String solution) {
        ...
        try {
            ...
            handleConnectionError(str("extenre_ryd_failure_generic", "confirm registration failed"), ex);
        } catch (Exception ex) {
    ...
    public static void sendVote(String userId, String videoId, ReturnYouTubeDislike.Vote vote) {
        ...
        try {
            ...
            handleConnectionError(str("extenre_ryd_failure_generic", "send vote failed"), ex);
        } catch (Exception ex) {
            // should never happen
    ...
    private static void confirmVote(String videoId, String userId, String solution) {
        ...
        try {
            ...
            handleConnectionError(str("extenre_ryd_failure_generic", "confirm vote failed"), ex);
        } catch (Exception ex) {
